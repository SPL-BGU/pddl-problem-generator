...
def generate_instance(
    instance_name: str, num_boats: int, num_people: int, max_dist_goals: int
) -> str:
    ...
    template_mapping = {
        ...
        "boat_velocity": "\n".join([f"(= (v b{i}) 1)" for i in range(num_boats)]),
        "people_name_list": " ".join([f"p{i}" for i in range(num_people)]),
        "people_d_position": "\n".join(
            [
                f"(= (d p{i}) {random.randint(-100, max_dist_goals)})"
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--min_boats", type=int, required=True, help="Starting numer of boats"
    )
    parser.add_argument(
        "--max_boats", type=int, required=True, help="Maximal number of boats"
    )
    parser.add_argument(
        "--min_people", type=int, required=True, help="Min of people to save"
    )
    parser.add_argument(
        "--max_people", type=int, required=True, help="Max of people to save"
    )
    parser.add_argument(
        "--max_dist_goal",
        type=int,
        required=False,
        help="Max distance people to be rescued",
        default=500,
    )
    parser.add_argument(
        "--output_folder",
...
def generate_multiple_problems(
    ...
    for i in range(total_number_problems):
        ...
        with open(output_folder / f"pfile{i}.pddl", "wt") as problem_file:
            problem_file.write(
                generate_instance(
                    f"instance_{num_boats}_{num_people}",
                    num_boats,
                    num_people,
                    max_dist_goal,
                )
